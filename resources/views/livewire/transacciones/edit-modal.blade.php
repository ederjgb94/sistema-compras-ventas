...
new class extends Component {
    ...
    public function mount(Transaccion $transaccion)
    {
        ...
        $this->contacto_id = $this->transaccion->contacto_id;
        $this->referencia_tipo = $this->transaccion->referencia_tipo;
        $this->referencia_nombre = $this->transaccion->referencia_nombre;
        $this->factura_tipo = $this->transaccion->factura_tipo;
        $this->factura_numero = $this->transaccion->factura_numero;
        $this->metodo_pago_id = $this->transaccion->metodo_pago_id;
        $this->referencia_pago = $this->transaccion->referencia_pago;
        $this->observaciones = $this->transaccion->observaciones;
        
        // Cargar conceptos si existen
        if ($this->transaccion->factura_datos && isset($this->transaccion->factura_datos['conceptos'])) {
            $this->conceptos = $this->transaccion->factura_datos['conceptos'];
        } else {
            $this->conceptos = [
                [
                    'descripcion' => '',
    ...
    public function save()
    {
        ...
        session()->flash('message', ucfirst($this->transaccion->tipo) . ' actualizado exitosamente.');

        $this->dispatch('transaccion-updated');
...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                Editar {{ ucfirst($transaccion->tipo) }}
            </h3>
    ...
    <form wire:submit="save" class="space-y-6">
                    ...
                    <div class="mt-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                        <div class="space-y-2">
                            ...
                            <div class="flex justify-between text-lg font-semibold border-t border-gray-200 dark:border-gray-600 pt-2">
                                ...
                                <span class="text-{{ $transaccion->tipo === 'ingreso' ? 'green' : 'red' }}-600 dark:text-{{ $transaccion->tipo === 'ingreso' ? 'green' : 'red' }}-400">
        ...
        <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
            ...
            <button type="submit" 
                    class="px-4 py-2 border border-transparent rounded-md text-sm font-medium text-white bg-{{ $transaccion->tipo === 'ingreso' ? 'green' : 'red' }}-600 hover:bg-{{ $transaccion->tipo === 'ingreso' ? 'green' : 'red' }}-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-{{ $transaccion->tipo === 'ingreso' ? 'green' : 'red' }}-500">
                Actualizar {{ ucfirst($transaccion->tipo) }}
            </button>
        </div>
    </form>
</div>
